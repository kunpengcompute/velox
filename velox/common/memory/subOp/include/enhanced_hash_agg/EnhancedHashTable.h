...
template <bool ignoreNullKeys>
class EnhancedHashTable : public BaseHashTable {
 public:
  ...
  ~EnhancedHashTable() override = default;

  enum class NormalizedKeyMode {nativeVelox, scalar, sve}; // TODO scalar2

  static std::unique_ptr<EnhancedHashTable> createForAggregation(
      std::vector<std::unique_ptr<VectorHasher>>&& hashers,
      const std::vector<Accumulator>& accumulators,
      memory::MemoryPool* pool) {
    return std::make_unique<EnhancedHashTable>(
        std::move(hashers),
        accumulators,
        std::vector<TypePtr>{},
  ...
  static std::unique_ptr<EnhancedHashTable> createForJoin(
      std::vector<std::unique_ptr<VectorHasher>>&& hashers,
      const std::vector<TypePtr>& dependentTypes,
      bool allowDuplicates,
      bool hasProbedFlag,
 ...
 private:
  ...
  std::vector<std::unique_ptr<EnhancedHashTable<ignoreNullKeys>>> otherTables_;

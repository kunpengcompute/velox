...
class EnhancedDecodedVector {
 public:
  /// Default constructor. The caller must call decode() or makeIndices() next.
  EnhancedDecodedVector() = default;

  /// Disable copy constructor and assignment.
  EnhancedDecodedVector(const EnhancedDecodedVector& other) = delete;
  EnhancedDecodedVector& operator=(const EnhancedDecodedVector& other) = delete;

  /// Allow std::move.
  EnhancedDecodedVector(EnhancedDecodedVector&& other) = default;
  ...
  EnhancedDecodedVector(const BaseVector& vector, bool loadLazy = true) {
...
template <>
inline bool EnhancedDecodedVector::valueAt(vector_size_t idx) const {
  return bits::isBitSet(reinterpret_cast<const uint64_t*>(data_), index(idx));
}

template <>
inline int128_t EnhancedDecodedVector::valueAt(vector_size_t idx) const {
  auto valuePosition =

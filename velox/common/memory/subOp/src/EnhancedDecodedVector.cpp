...
#include "velox/common/memory/subOp/include/enhanced_hash_agg/EnhancedDecodedVector.h"
...
uint64_t EnhancedDecodedVector::constantNullMask_{0};
...
const std::vector<vector_size_t>& EnhancedDecodedVector::consecutiveIndices() {
  static std::vector<vector_size_t> consecutiveIndices =
      makeConsecutiveIndices(10'000);
  return consecutiveIndices;
}

const std::vector<vector_size_t>& EnhancedDecodedVector::zeroIndices() {
  static std::vector<vector_size_t> indices(10'000);
  return indices;
}

void EnhancedDecodedVector::decode(
...
void EnhancedDecodedVector::makeIndices(
    const BaseVector& vector,
    const SelectivityVector* rows,
    int32_t numLevels) {
  if (rows) {
...
void EnhancedDecodedVector::reset(vector_size_t size) {
  if (!indicesNotCopied()) {
...
void EnhancedDecodedVector::copyNulls(vector_size_t size) {
...
void EnhancedDecodedVector::combineWrappers(
    const BaseVector* vector,
    const SelectivityVector* rows,
    int numLevels) {
...
void EnhancedDecodedVector::applyDictionaryWrapper(
...
void EnhancedDecodedVector::fillInIndices() {
  ...
  VELOX_FAIL(
      "EnhancedDecodedVector::indices_ must be set for non-constant non-consecutive mapping.");
}

void EnhancedDecodedVector::makeIndicesMutable() {
  if (indicesNotCopied()) {
...
void EnhancedDecodedVector::setFlatNulls(
    const BaseVector& vector,
    const SelectivityVector* rows) {
  if (hasExtraNulls_) {
    if (nullsNotCopied()) {
...
void EnhancedDecodedVector::setBaseData(
    const BaseVector& vector,
    const SelectivityVector* rows) {
...
void EnhancedDecodedVector::setBaseDataForConstant(
    const BaseVector& vector,
    const SelectivityVector* rows) {
  if (!vector.isScalar()) {
...
EnhancedDecodedVector::DictionaryWrapping EnhancedDecodedVector::dictionaryWrapping(
...
VectorPtr EnhancedDecodedVector::wrap(
    VectorPtr data,
    const BaseVector& wrapper,
    vector_size_t size) {
  if (isConstantMapping_) {
...
const uint64_t* EnhancedDecodedVector::nulls(const SelectivityVector* rows) {
  ...
  if (hasExtraNulls_) {
    ...
    if (isIdentityMapping_) {
      ...
      VELOX_CHECK(
          partialRowsDecoded_ == (rows != nullptr),
          "EnhancedDecodedVector::nulls() must be called with the same rows as decode()");
      if (rows != nullptr) {
...
void EnhancedDecodedVector::applyToRows(const SelectivityVector* rows, Func&& func)
    const {
  if (rows) {
...
std::string EnhancedDecodedVector::toString(vector_size_t idx) const {
  if (isNullAt(idx)) {
    return "null";
  }

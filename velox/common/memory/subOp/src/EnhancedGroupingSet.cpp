...
EnhancedGroupingSet::EnhancedGroupingSet(
...
std::unique_ptr<EnhancedGroupingSet> EnhancedGroupingSet::createForMarkDistinct(
    const RowTypePtr& inputType,
    std::vector<std::unique_ptr<VectorHasher>>&& hashers,
    OperatorCtx* operatorCtx,
    tsan_atomic<bool>* nonReclaimableSection) {
  return std::make_unique<EnhancedGroupingSet>(
      inputType,
      std::move(hashers),
...
void EnhancedGroupingSet::addInput(const RowVectorPtr& input, bool mayPushdown) {
  if (isGlobal_) {
...
void EnhancedGroupingSet::noMoreInput() {
  noMoreInput_ = true;
...
bool EnhancedGroupingSet::hasSpilled() const {
  if (inputSpiller_ != nullptr) {
    VELOX_CHECK_NULL(outputSpiller_);
    return true;
  }
...
bool EnhancedGroupingSet::hasOutput() {
  return noMoreInput_ || remainingInput_;
}

void EnhancedGroupingSet::addInputForActiveRows(
  ...
  TestValue::adjust(
      "facebook::velox::exec::EnhancedGroupingSet::addInputForActiveRows", this);
...
void EnhancedGroupingSet::addRemainingInput() {
...
std::vector<Accumulator> EnhancedGroupingSet::accumulators(bool excludeToIntermediate) {
...
void EnhancedGroupingSet::createHashTable() {
  if (ignoreNullKeys_) {
    table_ = EnhancedHashTable<true>::createForAggregation(
        std::move(hashers_), accumulators(false), &pool_);
  } else {
    table_ = EnhancedHashTable<false>::createForAggregation(
        std::move(hashers_), accumulators(false), &pool_);
  }
...
void EnhancedGroupingSet::initializeGlobalAggregation() {
...
void EnhancedGroupingSet::addGlobalAggregationInput(
    const RowVectorPtr& input,
    bool mayPushdown) {
...
bool EnhancedGroupingSet::getGlobalAggregationOutput(
...
bool EnhancedGroupingSet::getDefaultGlobalGroupingSetOutput(
    RowContainerIterator& iterator,
    RowVectorPtr& result) {
...
void EnhancedGroupingSet::destroyGlobalAggregations() {
  if (!globalAggregationInitialized_) {
    return;
  }
  for (int32_t i = 0; i < aggregates_.size(); ++i) {
...
void EnhancedGroupingSet::populateTempVectors(
...
const SelectivityVector& EnhancedGroupingSet::getSelectivityVector(
...
bool EnhancedGroupingSet::getOutput(
    int32_t maxOutputRows,
    int32_t maxOutputBytes,
    RowContainerIterator& iterator,
    RowVectorPtr& result) {
  TestValue::adjust("facebook::velox::exec::EnhancedGroupingSet::getOutput", this);
...
void EnhancedGroupingSet::extractGroups(
...
void EnhancedGroupingSet::resetTable(bool freeTable) {
  if (table_ != nullptr) {
    table_->clear(freeTable);
  }
}

bool EnhancedGroupingSet::isPartialFull(int64_t maxBytes) {
...
uint64_t EnhancedGroupingSet::allocatedBytes() const {
  if (table_) {
...
const HashLookup& EnhancedGroupingSet::hashLookup() const {
  return *lookup_;
}

void EnhancedGroupingSet::ensureInputFits(const RowVectorPtr& input) {
...
void EnhancedGroupingSet::ensureOutputFits() {
...
RowTypePtr EnhancedGroupingSet::makeSpillType() const {
  auto rows = table_->rows();
...
std::optional<common::SpillStats> EnhancedGroupingSet::spilledStats() const {
  if (!hasSpilled()) {
    return std::nullopt;
  }
  if (inputSpiller_ != nullptr) {
...
void EnhancedGroupingSet::spill() {
...
void EnhancedGroupingSet::spill(const RowContainerIterator& rowIterator) {
...
bool EnhancedGroupingSet::getOutputWithSpill(
    int32_t maxOutputRows,
    int32_t maxOutputBytes,
    const RowVectorPtr& result) {
...
bool EnhancedGroupingSet::prepareNextSpillPartitionOutput() {
...
bool EnhancedGroupingSet::mergeNext(
    int32_t maxOutputRows,
    int32_t maxOutputBytes,
    const RowVectorPtr& result) {
  if (isDistinct()) {
...
bool EnhancedGroupingSet::mergeNextWithAggregates(
    int32_t maxOutputRows,
    int32_t maxOutputBytes,
    const RowVectorPtr& result) {
  VELOX_CHECK(!isDistinct());
...
bool EnhancedGroupingSet::mergeNextWithoutAggregates(
    int32_t maxOutputRows,
    const RowVectorPtr& result) {
  VELOX_CHECK_NOT_NULL(merge_);
  VELOX_CHECK(isDistinct());
...
void EnhancedGroupingSet::initializeRow(SpillMergeStream& stream, char* row) {
...
void EnhancedGroupingSet::extractSpillResult(const RowVectorPtr& result) {
...
void EnhancedGroupingSet::updateRow(SpillMergeStream& input, char* row) {
...
void EnhancedGroupingSet::abandonPartialAggregation() {
...
void EnhancedGroupingSet::toIntermediate(
    const RowVectorPtr& input,
    RowVectorPtr& result) {
...
std::optional<int64_t> EnhancedGroupingSet::estimateOutputRowSize() const {
  if (table_ == nullptr) {
    return std::nullopt;
  }
